...
document.querySelector('#datePickerBtnYM').addEventListener('click', function () {
    weui.datePickerYM({
        start: '2016-12-29', //开始时间，new Date(), // 从今天开始
        end: '2030-12-29',//结束时间
        cron: '* * *',
        defaultValue: [2017, 7, 9],//默认选项的value数组,也就是默认选中的时间, 如 [1991, 6, 9]
        onChange: function (result) {
            console.log(result); //在picker选中的值发生变化的时候回调
        },
        onConfirm: function (result) {
            console.log(result); //在点击"确定"之后的回调。回调返回选中的结果(Array)，数组长度依赖于picker的层级。

            document.querySelector('#datePickerBtnYM').innerHTML = result[0].label+result[1].label; //返回结果
        },
        id: 'datePickerYM'
    });
});
